    ...
    def _draw(self, frame, boxes, probs, landmarks, names, confidences):
        if boxes is not None:
            for box, prob, ld, name, confidence in zip(boxes, probs, landmarks, names, confidences):
                cv2.rectangle(frame, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (0, 225, 0), thickness=2)
                cv2.putText(frame, f'{name} ({prob:.2f}, {confidence:.2f})', (int(box[0]), int(box[1]) - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 225, 0), 2, cv2.LINE_AA)
                for point in ld:
                    cv2.circle(frame, (int(point[0]), int(point[1])), 5, (0, 225, 0), -1)
        else:
...
train_video_path = '/Users/thapahemmagar/Documents/ord/Mtcnn_face_detection/animesh.mp4'
output_dir = 'dataset/train'
name = "Animesh"
...
test_video_path = '/Users/thapahemmagar/Documents/ord/Mtcnn_face_detection/dog1.mp4'
