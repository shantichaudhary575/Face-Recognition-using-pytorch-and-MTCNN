    ...
    for dirpath, dirnames, filenames in os.walk(directory):
        for filename in filenames:
            if filename.endswith(".jpg"):
                ...
                # Append the name and embedding
                known_face_names.append(os.path.basename(dirpath))  # Assuming the directory name is the label
                known_face_embeddings.append(embedding)

    # Check if known_face_names and known_face_embeddings are empty
...
def extract_faces_from_video(train_video_path, output_dir, max_frames=200):
    ...
    frame_count = 0
    while cap.isOpened() and frame_count < max_frames:  # Add condition for maximum frames
        ret, frame = cap.read()
        if not ret:
            break
...
extract_faces_from_video(train_video_path, output_dir, max_frames=200)  # Call function with max_frames parameter




# Load known face embeddings and names from the saved images
known_face_names, known_face_embeddings = load_known_faces_from_directory(output_dir)

# Ensure embeddings are in tensor format
...
# Run on test video
test_video_path = '/Users/thapahemmagar/Documents/ord/Mtcnn_face_detection/shantii_test.mp4'
